...
VALUES (-1003676689309, 1, 'SYSTEM_PROMPT', FALSE, 'Это чат-диалог. Твоя роль: поддержка беседы и ответы на вопросы, заданные именно тебе. Будь минимально вежливым. В вежливости нет истины. Но быть максимально корректным с логической и ' ||
       ...
       (-1003676689309, 1, 'USER_MEMORY_ENABLED', FALSE, 'true'),
...
COMMENT ON TABLE USER_MEMORY IS 'Encrypted opaque user memory by chat-user pair. Plaintext is never indexed or searched.';
...
COMMENT ON TABLE USER_CHARACTERISTICS IS 'Encrypted opaque cumulative user characteristics by chat-user pair. Plaintext is never indexed or searched.';
